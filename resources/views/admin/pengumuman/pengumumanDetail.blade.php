                    ...
                    <div class="d-flex flex-md-row flex-sm-column w-auto justify-content-start align-items-center">
                        ...
                        <div class="align-self-start">
                            ...
                            <button type="button" id="btn-hapus" class="btn align-items-center"
                                onclick="openDeleteModal({{ $pengumuman->id }}, '{{ $pengumuman->judul_berita }}')">
                    ...
                    @if (!empty($lampiran) && is_array($lampiran) && count($lampiran))
                        <ul style="font-size: 1rem; color: white; list-style: none; padding: 0;">
                            @foreach ($lampiran as $file)
                                @php
                                    // $file menyimpan string seperti "assets/data/nama_file.ext"
                                    $fullPath = public_path($file);
                                @endphp
                                @if (file_exists($fullPath))
                                    <li style="margin-bottom: 10px;">
                                        <a href="{{ asset($file) }}" target="_blank" style="color: white;">
                                            {{ basename($file) }}
                                        </a>
                                    </li>
                                @else
