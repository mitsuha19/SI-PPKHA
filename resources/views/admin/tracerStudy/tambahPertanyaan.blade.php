    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('change', '.question-type', function() {
                ...
                $optionsContainer.find('.option-row, .scale-options').remove();

                // Show options container for types that need it
                if ([3, 4, 5, 6].includes(typeId)) { // Pilihan Ganda, Kotak Centang, Dropdown, Skala Linier
                    $optionsContainer.show();

                    // For Linear Scale, add start and end scale options
                    if (typeId === 6) { // Skala Linier
                    ...
                    // For Multiple Choice, add one option with section selection
                    else if (typeId === 3) { // Pilihan Ganda
                        addMultipleChoiceOption(sectionIndex, questionIndex, 0, $optionsContainer);
                        updateAvailableSections(sectionIndex);
                    }
                    // For other option types, add empty option
                    else {
