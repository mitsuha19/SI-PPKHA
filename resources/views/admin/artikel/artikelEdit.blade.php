    ...
    <div class="main-content">
        ...
        <div class="box-form">
            <form action="{{ route('admin.artikel.update', $artikel->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <textarea class="form-control mb-3" id="deskripsi" name="deskripsi_artikel" placeholder="Masukkan deskripsi Artikel"
                    required>
          {{ $artikel->deskripsi_artikel }}
        </textarea>

                <label for="sumber" class="form-label poppins-bold text-black mb-2">Sumber Artikel:</label>
                <input class="form-control mb-3" id="sumber" name="sumber_artikel"
                    value="{{ $artikel->sumber_artikel }}"></input>
                                ...
                                @if (is_array($gambarArray))
                                    @foreach ($gambarArray as $index => $file)
                                        <div id="lampiran-item-{{ $index }}" class="d-flex align-items-center mb-2">
                                            <img src="{{ asset($file) }}" alt="Gambar Artikel" width="100"
                                                class="me-2">
                                            <label class="ms-2">
                                                <input type="checkbox" name="hapus_gambar[]" value="{{ $file }}">
                                                Hapus
                                            </label>
                                        </div>
                                    @endforeach
